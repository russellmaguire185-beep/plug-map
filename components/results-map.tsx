...
function FitBounds({
  ...
  useEffect(() => {
    const validPoints = locations
      ...
      .map((location) => [location.lat as number, location.lng as number] as [number, number])
...
export default function ResultsMap({ locations }: ResultsMapProps) {
  ...
  const mappableLocations = useMemo(
    () => locations.filter((location) => location.lat !== null && location.lng !== null),
    [locations]
  )
  ...
  if (mappableLocations.length === 0) {
    return (
      <div className="rounded-[2rem] border border-white/20 bg-white/10 p-6 text-white/80 backdrop-blur-xl">
        No mapped locations available yet.
      </div>
    )
  }
          ...
          >
            <Popup>
              <div className="max-w-[260px] space-y-2 text-sm text-slate-900">
                ...
                {(location.hub_code || location.terminal || location.near_gate || location.train_platform) && (
                  <div className="text-xs text-slate-700">
